...
export async function ensureIndexes(): Promise<void> {
  const stmts = [
    ...
    // Persistent single-use score voucher ledger. The UNIQUE JTI is the
    // authoritative replay guard across restarts and multiple replicas.
    `CREATE TABLE IF NOT EXISTS score_voucher_uses (
       id serial PRIMARY KEY,
       jti text NOT NULL,
       expires_at timestamp NOT NULL,
       used_at timestamp NOT NULL DEFAULT NOW()
     )`,
    `CREATE UNIQUE INDEX IF NOT EXISTS score_voucher_uses_jti_uidx
       ON score_voucher_uses (jti)`,
    `CREATE INDEX IF NOT EXISTS score_voucher_uses_expires_at_idx
       ON score_voucher_uses (expires_at)`,
  ...
  for (const stmt of stmts) {
    try {
      ...
      // IF NOT EXISTS is race-safe for normal boots; tolerate duplicate errors
      // from two cold starts racing on an older PostgreSQL version.
