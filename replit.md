...
- **Retention pass — contextual upsells + at-risk streak (no new ads)**: replaced the generic per-round `BannerAd` in `SoloGame.tsx` (end-of-round summary) with a new `ContextualPremiumPrompt` (`artifacts/stop-game/src/components/ContextualPremiumPrompt.tsx`) that picks a tailored CTA based on the player's situation (close loss → "espía 2x te habría salvado"; spy exhausted → "premium tiene 2 espías"; streak ≥ 7 → "ganaste el anillo dorado"; generic loss → "quita anuncios"). Falls back to the original `BannerAd` when no context is strong, so ad revenue is preserved on neutral moments. The streak badge in `Home.tsx` is now a `Link` to `/solo?mode=quick&auto=1` and shows a pulsing red "⚠️ Salva tu racha" state when `streak.current > 0 && !playedToday`. Followups still pending in Task #1: streak calendar with rewards, smart push cron, season pass, FTUE difficulty ramp, contextual upsells in `Stripe trial period`, analytics events.
- **Premium "2x espía" promise honored in Solo mode too**: `SoloGame.tsx` previously hard-coded a single spy use (`spyUsed: boolean`). Multiplayer already enforced 2x for premium server-side, but Solo silently capped premium users at 1 use → mismatch with the Play Store listing. New design uses TWO counters: `spyUsesLeft` (per-game pool, initialized in `startGame()` to `isPremium ? 2 : 1`) and `spyUsesThisRound` (resets in `startRound()` so the -10 pt penalty is only applied in the round where the spy was actually used — the original `spyUsed ? 10 : 0` logic mistakenly re-applied the cost in every subsequent round). Premium UI shows a `⭐ N/2` badge so the extra use is visible.
- **Daily Challenge 0pts bug fixed (race with stale state)**: in `SoloGame.tsx`, the auto-submit `useEffect` read `totalScore` from a closure that was still stale because `setTotalScore(finalPlayerScore)` (called in the round-results effect) hadn't propagated yet — both effects ran in the SAME render cycle when `gameState` flipped to `RESULTS`. Result: the daily ranking always saved the score *before* the last round (typically 0 for single-round dailies). Fix: submit INLINE inside the round-results effect's `if (round >= maxRounds)` block, using the freshly computed `finalPlayerScore` and `aiTotalScore + as_` directly. Same fix also corrects the regular leaderboard, which was silently losing the last round's points. The auto-submit `useEffect` is kept as a safety net (its `submittedRef` guard prevents double submission).
- **Animal dictionary holes patched**: validator was rejecting super-common animals (gato, león, rata, mosca, calamar, gusano, lagarto, escarabajo, libélula, dragón, dinosaurio, chimpancé, mejillón…) because they were missing from the Spanish dictionary in `artifacts/api-server/src/routes/game.ts`. Added them to all 4 language lists (es/en/pt/fr) so word validation no longer scores 0pts on obvious correct answers.
