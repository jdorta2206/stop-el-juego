...
// Native Android rewarded Activity reports its result here. Results are short-lived,
// keyed by a cryptographically random request id generated by the web client.
// This is deliberately separate from player reward state: the web client remains
// responsible for applying the game-specific reward only after rewarded=true.
type AdmobResult = { rewarded: boolean; createdAt: number };
const admobResults = new Map<string, AdmobResult>();
const ADMOB_RESULT_TTL_MS = 2 * 60 * 1000;
const ADMOB_REQUEST_ID_RE = /^[A-Za-z0-9_-]{20,128}$/;

function pruneAdmobResults(now = Date.now()): void {
  for (const [id, result] of admobResults) {
    if (now - result.createdAt > ADMOB_RESULT_TTL_MS) admobResults.delete(id);
  }
}

router.post("/admob-result", (req, res) => {
  pruneAdmobResults();
  ...
  admobResults.set(requestId, { rewarded, createdAt: Date.now() });
  res.status(204).end();
});

router.get("/admob-result/:requestId", (req, res) => {
  pruneAdmobResults();
  ...
  const result = admobResults.get(requestId);
  if (!result) {
    res.json({ ready: false });
    return;
  }
  admobResults.delete(requestId);
  res.json({ ready: true, rewarded: result.rewarded });
});
