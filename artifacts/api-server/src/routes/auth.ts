...
// Canonical origin where OAuth runs.
const APP_ORIGIN = process.env["APP_ORIGIN"] || "https://stop-el-juego.replit.app";

// Safe origins for OAuth return redirects.
...
function verifyAuthState(req: Request, res: Response): { returnPath: string; returnOrigin: string; csrfFail: boolean } {
  ...
  if (nonceCookie) {
    ...
    let match = false;
    try { match = nonceCookie.length === signed.n.length && crypto.timingSafeEqual(Buffer.from(nonceCookie), Buffer.from(signed.n)); }
    catch { match = false; }
...
router.get("/google/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (tokenData.error) { console.error("Google token error:", tokenData.error, tokenData.error_description); throw new Error(`Google error: ${tokenData.error} - ${tokenData.error_description}`); }
    ...
    else if (tokenData.access_token) { const userinfoRes = await fetch("https://www.googleapis.com/oauth2/v3/userinfo", { headers: { Authorization: `Bearer ${tokenData.access_token}` } }); payload = (await userinfoRes.json()) as OAuthProfile; console.log("Google userinfo payload:", JSON.stringify(payload)); }
    else throw new Error("No id_token or access_token in Google response");
    if (!payload?.sub) throw new Error("Google response missing subject");
  ...
  } catch (err) { console.error("Google OAuth error:", err); return res.redirect(`${APP_ORIGIN}/?auth_error=google_failed`); }
});
...
router.get("/facebook/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!me.id) throw new Error("Facebook response missing id");
  ...
  } catch (err) { console.error("Facebook OAuth error:", err); return res.redirect(`${APP_ORIGIN}/?auth_error=facebook_failed`); }
});
...
router.get("/instagram/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (tokenData.error_type || tokenData.error) throw new Error(`Instagram error: ${tokenData.error_type || tokenData.error} - ${tokenData.error_message || tokenData.error_description}`);
    if (!tokenData.access_token) throw new Error("No access_token from Instagram");
    const meRes = await fetch(`https://graph.instagram.com/v21.0/me?fields=id,username,profile_picture_url&access_token=${tokenData.access_token}`);
    const me = (await meRes.json()) as OAuthProfile;
    ...
    const sessionToken = issuePlayerToken(res, playerId);
    return res.send(bridgePageMulti([["oauth_user", user], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) { console.error("Instagram OAuth error:", err); return res.redirect(`${APP_ORIGIN}/?auth_error=instagram_failed`); }
});
...
router.post("/apple/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const tokenData = (await tokenRes.json()) as OAuthTokenResponse;
    if (tokenData.error || !tokenData.id_token) throw new Error("Apple OAuth error");
    const payload = JSON.parse(Buffer.from(tokenData.id_token.split(".")[1], "base64url").toString());
    let displayName = "Apple User";
    try { const userJson = req.body?.["user"]; if (userJson) { const u = typeof userJson === "string" ? JSON.parse(userJson) : userJson; const firstName = u?.name?.firstName || ""; const lastName = u?.name?.lastName || ""; displayName = `${firstName} ${lastName}`.trim() || displayName; } } catch (_) {}
    ...
    const sessionToken = issuePlayerToken(res, playerId);
    return res.send(bridgePageMulti([["oauth_user", user], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) { console.error("Apple OAuth error:", err); return res.redirect(`${APP_ORIGIN}/?auth_error=apple_failed`); }
});
...
router.get("/tiktok/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const sessionToken = issuePlayerToken(res, playerId);
    return res.send(bridgePageMulti([["oauth_user", user], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) { console.error("TikTok OAuth error:", err); return res.redirect(`${APP_ORIGIN}/?auth_error=tiktok_failed`); }
});
...
router.get("/me", async (req: Request, res: Response) => {
  ...
  } catch (err) { console.error("[auth/me] error:", err); return res.status(500).json({ error: "Internal error" }); }
});

router.post("/logout", (_req: Request, res: Response) => { clearPlayerToken(res); res.json({ ok: true }); });
