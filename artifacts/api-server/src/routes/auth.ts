...
function parseSignedState(raw: string | undefined): { r: string; o: string; n: string; t: number } | null {
  ...
  let ok = false;
  try { ok = sig.length === expected.length && crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expected)); } catch { return null; }
  if (!ok) return null;
  try {
    ...
    if (typeof p["r"] === "string" && typeof p["n"] === "string" && typeof p["t"] === "number") {
      const o = typeof p["o"] === "string" && SAFE_RETURN_ORIGINS.has(p["o"]) ? p["o"] as string : APP_ORIGIN;
...
function verifyAuthState(req: Request, res: Response): { returnPath: string; returnOrigin: string; csrfFail: boolean } {
  ...
  if (nonceCookie) {
    ...
    let match = false;
    try { match = nonceCookie.length === signed.n.length && crypto.timingSafeEqual(Buffer.from(nonceCookie), Buffer.from(signed.n)); } catch { match = false; }
...
<style>body{background:#0d1757;color:white;font-family:sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;}.logo{text-align:center;}.spinner{width:40px;height:40px;border:4px solid rgba(255,255,255,.2);border-top-color:#f9a825;border-radius:50%;animation:spin 0.8s linear infinite;margin:16px auto;}@keyframes spin{to{transform:rotate(360deg)}}</style></head>
...
router.get("/google/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (tokenData.error) throw new Error(`Google error: ${tokenData.error} - ${tokenData.error_description}`);
    ...
    else if (tokenData.access_token) payload = (await (await fetch("https://www.googleapis.com/oauth2/v3/userinfo", { headers: { Authorization: `Bearer ${tokenData.access_token}` } })).json()) as OAuthProfile;
    else throw new Error("No id_token or access_token in Google response");
    if (!payload?.sub) throw new Error("Google profile missing sub");
    ...
    return res.send(bridgePageMulti([["oauth_user", user], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) {
    console.error("Google OAuth error:", err);
    return res.redirect(`${APP_ORIGIN}/?auth_error=google_failed`);
  }
});
...
router.get("/facebook/start", (req: Request, res: Response) => {
  ...
  const returnOrigin = pickReturnOrigin(req, requestedOrigin);
  const state = beginAuthState(res, returnPath, returnOrigin);
  const params = new URLSearchParams({ client_id: FACEBOOK_APP_ID, redirect_uri: redirectUri, scope: "email,public_profile", response_type: "code", state });
  return res.redirect(`https://www.facebook.com/v26.0/dialog/oauth?${params.toString()}`);
});
...
router.get("/facebook/callback", async (req: Request, res: Response) => {
  ...
  try {
    const redirectUri = `${APP_ORIGIN}/api/auth/facebook/callback`;
    const tokenUrl = `https://graph.facebook.com/v26.0/oauth/access_token?${new URLSearchParams({ client_id: FACEBOOK_APP_ID, redirect_uri: redirectUri, client_secret: FACEBOOK_APP_SECRET, code }).toString()}`;
    const tokenRes = await fetch(tokenUrl);
    const tokenData = (await tokenRes.json()) as OAuthTokenResponse;
    if (tokenData.error || !tokenData.access_token) throw new Error(`Facebook token error: ${tokenData.error || "No access_token"} ${tokenData.error_description || tokenData.error_message || ""}`);
    const meUrl = `https://graph.facebook.com/v26.0/me?${new URLSearchParams({ fields: "id,name,email,picture.type(large)", access_token: tokenData.access_token }).toString()}`;
    const meRes = await fetch(meUrl);
    const me = (await meRes.json()) as OAuthProfile;
    if (me.error || !me.id) throw new Error(`Facebook profile error: ${JSON.stringify(me)}`);
    ...
    const sessionToken = issuePlayerToken(res, playerId);
    return res.send(bridgePageMulti([["oauth_user", user], ["fb_access_token", tokenData.access_token], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) {
    console.error("Facebook OAuth error:", err);
    return res.redirect(`${APP_ORIGIN}/?auth_error=facebook_failed`);
  }
});

router.get("/instagram/start", (req: Request, res: Response) => {
  const INSTAGRAM_CLIENT_ID = process.env["INSTAGRAM_CLIENT_ID"];
  if (!INSTAGRAM_CLIENT_ID) return res.redirect(`${APP_ORIGIN}/?auth_error=instagram_not_configured`);
  const redirectUri = `${APP_ORIGIN}/api/auth/instagram/callback`;
  const returnPath = (req.query["return"] as string) || "/";
  const returnOrigin = pickReturnOrigin(req, null);
  const state = beginAuthState(res, returnPath, returnOrigin);
  const params = new URLSearchParams({ client_id: INSTAGRAM_CLIENT_ID, redirect_uri: redirectUri, scope: "instagram_business_basic", response_type: "code", state });
...
router.get("/instagram/callback", async (req: Request, res: Response) => {
  ...
  if (error || !code) return res.redirect(`${APP_ORIGIN}/?auth_error=instagram_cancelled`);
  if (!INSTAGRAM_CLIENT_ID || !INSTAGRAM_CLIENT_SECRET) return res.redirect(`${APP_ORIGIN}/?auth_error=instagram_not_configured`);
  if (!claimCode(`ig_${code}`)) return res.redirect(`${returnOrigin}${returnPath}`);
  try {
    const redirectUri = `${APP_ORIGIN}/api/auth/instagram/callback`;
    const tokenRes = await fetch("https://api.instagram.com/oauth/access_token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: INSTAGRAM_CLIENT_ID, client_secret: INSTAGRAM_CLIENT_SECRET, grant_type: "authorization_code", redirect_uri: redirectUri, code }) });
    const tokenData = (await tokenRes.json()) as OAuthTokenResponse;
    if (tokenData.error_type || tokenData.error || !tokenData.access_token) throw new Error(`Instagram error: ${tokenData.error_type || tokenData.error || "No access_token"}`);
    const me = (await (await fetch(`https://graph.instagram.com/v21.0/me?${new URLSearchParams({ fields: "id,username,profile_picture_url", access_token: tokenData.access_token }).toString()}`)).json()) as OAuthProfile;
    if (!me.id) throw new Error(`Instagram profile error: ${JSON.stringify(me)}`);
    const playerId = `ig_${me.id}`;
    const user = JSON.stringify({ id: playerId, name: me.username || me.name || "Usuario", picture: me.profile_picture_url || null, provider: "instagram" });
    const sessionToken = issuePlayerToken(res, playerId);
    return res.send(bridgePageMulti([["oauth_user", user], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) {
    console.error("Instagram OAuth error:", err);
    return res.redirect(`${APP_ORIGIN}/?auth_error=instagram_failed`);
  }
});

function makeAppleClientSecret(): string {
  const privateKey = (process.env["APPLE_PRIVATE_KEY"] || "").replace(/\\n/g, "\n");
  return jwt.sign({}, privateKey, { algorithm: "ES256", keyid: process.env["APPLE_KEY_ID"]!, issuer: process.env["APPLE_TEAM_ID"]!, audience: "https://appleid.apple.com", subject: process.env["APPLE_CLIENT_ID"]!, expiresIn: "5m" });
}
...
router.post("/apple/callback", async (req: Request, res: Response) => {
  ...
  try {
    const redirectUri = `${APP_ORIGIN}/api/auth/apple/callback`;
    const tokenRes = await fetch("https://appleid.apple.com/auth/token", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: APPLE_CLIENT_ID, client_secret: makeAppleClientSecret(), code, grant_type: "authorization_code", redirect_uri: redirectUri }) });
    const tokenData = (await tokenRes.json()) as OAuthTokenResponse;
    if (tokenData.error || !tokenData.id_token) throw new Error(`Apple error: ${tokenData.error || "No id_token"}`);
    const payload = JSON.parse(Buffer.from(tokenData.id_token.split(".")[1], "base64url").toString());
    let displayName = "Apple User";
    try { const u = req.body?.["user"]; if (u) { const parsed = typeof u === "string" ? JSON.parse(u) : u; displayName = `${parsed?.name?.firstName || ""} ${parsed?.name?.lastName || ""}`.trim() || displayName; } } catch (_) {}
...
router.get("/tiktok/callback", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const meData = (await (await fetch("https://open.tiktokapis.com/v2/user/info/?fields=open_id,display_name,avatar_url", { headers: { Authorization: `Bearer ${accessToken}` } })).json()) as TikTokUserResponse;
    const me = meData.data?.user || meData.user || {};
    const playerId = `tt_${me.open_id || openId}`;
    if (!playerId || playerId === "tt_undefined") throw new Error("TikTok profile missing open_id");
    const user = JSON.stringify({ id: playerId, name: me.display_name || "TikToker", picture: me.avatar_url || null, provider: "tiktok" });
    const sessionToken = issuePlayerToken(res, playerId);
    return res.send(bridgePageMulti([["oauth_user", user], ...(sessionToken ? [[PLAYER_TOKEN_BRIDGE_KEY, sessionToken] as [string, string]] : [])], returnPath, returnOrigin));
  } catch (err) {
    console.error("TikTok OAuth error:", err);
    return res.redirect(`${APP_ORIGIN}/?auth_error=tiktok_failed`);
  }
});
