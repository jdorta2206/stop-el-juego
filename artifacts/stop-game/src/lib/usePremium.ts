...
export function usePremium(playerId: string | null | undefined): PremiumStatus & { playProduct: PlayPremiumProduct | null } {
        ...
        if (!cancelled && product) {
          setPlayProduct({
            ...
            priceLabel: product.price
              ? new Intl.NumberFormat("es-ES", { style: "currency", currency: product.price.currency }).format(Number(product.price.value))
              : undefined,
          });
        }
      } catch {}
    };
    void detectPaymentChannel().then((channel) => {
      if (channel === "play") void loadPlayProduct();
    }).catch(() => {});
  ...
  useEffect(() => {
    ...
    void detectPaymentChannel().then(async (channel) => {
      if (channel === "play") {
        await restorePlayPurchases(playerId).catch(() => undefined);
      }
      const endpoint = channel === "play"
      ...
      const response = await fetch(endpoint, { credentials: "include", headers: authHeaders() });
      if (!response.ok) throw new Error(`Premium status HTTP ${response.status}`);
      return response.json() as Promise<{ isPremium?: boolean }>;
    }).then((data) => {
