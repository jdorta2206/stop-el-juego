...
export function CosmeticShop() {
  ...
  const handleBuyPack = useCallback(async () => {
    ...
    try {
      ...
      window.alert("❌ No se pudo completar la compra con Google Play. Inténtalo de nuevo.");
    } catch (error: any) {
            ...
            >
              <div className="text-center">
                ...
                <div className="flex gap-3">
                  <Button onClick={() => setShowPackModal(false)} variant="outline" className="flex-1">Cancelar</Button>
                  <Button onClick={handleBuyPack} className="flex-1 bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-bold">Comprar</Button>
                </div>
              </div>
            </motion.div>
          </motion.div>
