...
const PRESTIGE_MIN = 200;
const PRESTIGE_STEP = 100;
      ...
      <div className="flex-1 flex flex-col max-w-md mx-auto w-full py-6 gap-6 pb-10">
        ...
        <div className="space-y-2 px-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-white/50">{data.gamesPlayed} partidas</span>
          </div>
          ...
          {nextLevel && (
            <p className="text-xs text-white/40 text-right">
              Siguiente: {nextLevel.label} ({nextLevel.min - data.gamesPlayed} partidas)
            </p>
          )}
        </div>
        ...
        {modeKeys.length > 0 && (
          <div className="flex flex-wrap gap-2 justify-center">
            {modeKeys.map(m => (
              <span key={m} className="px-3 py-1 rounded-full text-xs font-bold bg-white/10 text-white/70">
                {MODE_LABELS[m]?.icon ?? "🎮"} {MODE_LABELS[m]?.label ?? m}
              </span>
            ))}
          </div>
        )}
