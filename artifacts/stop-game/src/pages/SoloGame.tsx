...
export default function SoloGame() {
  ...
  const getHintWord = async (category: string): Promise<string> => {
    ...
    const findWords = (dict: Record<string, string[]>, categoryName: string): string[] => {
      ...
      const fuzzy = Object.entries(dict)
        .filter(([key]) => {
    ...
    const pick = (dict: Record<string, string[]>) => findWords(dict, requestedCategory)
      .filter(Boolean)
      .filter(word => normalize(word).startsWith(normalize(normalizedLetter)))
    ...
    // Never fall back to just the letter: a hint should provide a complete
    // valid answer that the player can use to score.
    return "";
  };
    ...
    if (rewardedAdType === "extraTime") {
      ...
      if (empty) {
        ...
        if (word) {
          ...
          toast({
            title: lang === "en" ? "No valid hint available for this category" :
              lang === "pt" ? "Não há pista válida disponível para esta categoria" :
              lang === "fr" ? "Aucun indice valide disponible pour cette catégorie" :
              "No hay una pista válida disponible para esta categoría",
          });
        }
      }
      setHintUsed(true);
    } else if (rewardedAdType === "double") {
      // Doubling: the original totalScore was already submitted when the final
      // results screen appeared. The bonus is submitted separately.
      setTotalScore(prev => {
        const bonus = prev;
