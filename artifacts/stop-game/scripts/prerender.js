...
const pages = [
  {
    ...
    description:
      "Juega a STOP online gratis. Completa categorías con palabras que empiecen por la letra indicada, compite contra amigos o la IA y descubre nuevas formas de jugar.",
    content: `
...
for (const page of pages) {
  ...
  if (!rootMatch) {
    throw new Error("No se pudo localizar el contenedor #root completo en dist/index.html.");
  }

  const replacement = `<div id="root">${content}</div>`;
  html = html.replace(rootPattern, replacement);
...
const root = finalHtml.match(/<div\s+id=["']root["']>([\\s\\S]*?)<\/div>/i)?.[1] ?? "";
