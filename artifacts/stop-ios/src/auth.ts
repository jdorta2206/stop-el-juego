...
export async function loadSession(): Promise<StopSession | null> {
  ...
  if (!raw) return null;
  try { return JSON.parse(raw) as StopSession; }
...
async function finishWebOAuth(resultUrl: string, provider: "google" | "facebook"): Promise<StopSession> {
  ...
  const raw = params.get("stopauth");
  if (!raw) throw new Error(`No se recibió la sesión de ${provider}.`);

  let items: unknown;
  try { items = JSON.parse(decodeURIComponent(raw)); }
  catch { throw new Error("Respuesta de autenticación no válida."); }
  if (!Array.isArray(items)) throw new Error("Respuesta de autenticación no válida.");

  const map = new Map<string, string>();
  for (const item of items) {
    if (Array.isArray(item) && typeof item[0] === "string" && typeof item[1] === "string") map.set(item[0], item[1]);
  }

  const token = map.get("stop_player_token");
  ...
  try { user = JSON.parse(rawUser) as StopSession["user"]; }
  catch { throw new Error("Perfil de usuario no válido."); }
...
/** OAuth Google/Facebook through the existing STOP backend. The provider secret
 * never reaches iOS; the backend performs the code exchange and issues the
 * normal STOP player token. */
...
export async function signInWithApple(): Promise<StopSession> {
  ...
  const response = await fetch(`${API_BASE_URL}/api/auth/apple/native`, {
    ...
    body: JSON.stringify({ identityToken: credential.identityToken, nonce, user: credential.user, email: credential.email, name: fullName || undefined }),
  });
  if (!response.ok) {
