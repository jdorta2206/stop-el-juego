...
type Player = { playerId: string; playerName: string; avatarColor?: string | null; score?: number; roundScore?: number; isHost?: boolean; isReady?: boolean; finishedAt?: number; wasStopper?: boolean };
type Room = { roomCode: string; hostId: string; hostName: string; status: string; currentLetter?: string | null; currentRound?: number | null; maxRounds?: number; maxPlayers?: number; gameMode?: string; language?: string; players?: Player[]; roundStartedAt?: number | null; roundEndsAt?: number | null; roundDurationSecs?: number; serverNow?: number; stopper?: { id?: string; name?: string; stopTimestamp?: number } | null; rematchCode?: string | null };
...
export function MultiplayerScreen({ session, onExit }: { session: StopSession; onExit: () => void }) {
  ...
  const refreshRoom = useCallback(async (roomCode: string) => {
    try {
      ...
      setRoom(data);
      if (data.rematchCode) setRematchCode(data.rematchCode);
      return data;
    } catch (e) {
      setError(errorMessage(e, "No se pudo actualizar la sala."));
      return null;
    }
  }, [player.playerId]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { void refreshRoom(room.roomCode); }, 1000);
    return () => clearInterval(timer);
  }, [room?.roomCode, refreshRoom]);

  useEffect(() => {
    if (room?.status !== "playing" || !room.roundEndsAt) { setSeconds(0); return; }
    const offset = (room.serverNow ?? Date.now()) - Date.now();
    const tick = () => setSeconds(Math.max(0, Math.ceil((room.roundEndsAt! - (Date.now() + offset)) / 1000)));
    tick();
  ...
  useEffect(() => {
    ...
    if (roundKey !== expected) {
      setRoundKey(expected);
      setAnswers({});
      setSubmitted(false);
      setError(null);
    }
  ...
  useEffect(() => {
    if (room?.status === "playing" && seconds === 0 && !submitted && !busy) void submitResults(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [seconds, room?.status, submitted, busy]);

  async function createRoom() {
    setBusy(true); setError(null);
    try {
      const created = await apiFetch<Room>("/api/rooms", { method: "POST", body: JSON.stringify({ hostId: player.playerId, hostName: player.playerName, avatarColor: player.avatarColor, maxRounds: 3, language: "es", loginMethod: player.loginMethod, isPublic: false, gameMode: "classic", maxPlayers: 8 }) });
      setRoom(created); setCode(created.roomCode);
    } catch (e) { setError(errorMessage(e, "No se pudo crear la sala.")); }
  ...
  async function joinRoom() {
    ...
    setBusy(true); setError(null);
    try {
      const joined = await apiFetch<Room>(`/api/rooms/${encodeURIComponent(roomCode)}/join`, { method: "POST", body: JSON.stringify(player) });
      setRoom(joined); setCode(joined.roomCode);
    } catch (e) { setError(errorMessage(e, "No se pudo entrar en la sala.")); }
    finally { setBusy(false); }
  }
  ...
  async function shareRoom() {
    if (!room?.roomCode) return;
    try {
      await Share.share({ title: "Únete a mi partida de STOP", message: `🎮 ¡Únete a mi partida de STOP!\n\nCódigo de sala: ${room.roomCode}\n\nJuega conmigo desde iPhone, Android o Web.` });
    } catch (e) { setError(errorMessage(e, "No se pudo abrir el menú de compartir.")); }
  }
  ...
  async function startRound() {
    ...
    setBusy(true); setError(null);
    try {
      const started = await apiFetch<Room>(`/api/rooms/${encodeURIComponent(room.roomCode)}/start`, { method: "POST", body: JSON.stringify({ hostId: player.playerId }) });
      setRoom(started); setAnswers({}); setSubmitted(false); setRoundKey(`${started.roomCode}:${started.currentRound}`);
    } catch (e) { setError(errorMessage(e, "No se pudo iniciar la ronda.")); }
    finally { setBusy(false); }
  }

  async function submitResults(stopFirst = true) {
    if (!room || submitted || busy || room.status !== "playing") return;
    setBusy(true); setError(null);
    try {
    ...
    } catch (e) { setError(errorMessage(e, "No se pudieron enviar las respuestas.")); }
    finally { setBusy(false); }
  }

  async function requestRematch() {
    if (!room?.roomCode || rematchLoading) return;
    if (rematchCode) {
      setCode(rematchCode);
      setRoom(null);
      setError(null);
      try {
        const joined = await apiFetch<Room>(`/api/rooms/${encodeURIComponent(rematchCode)}/join`, { method: "POST", body: JSON.stringify(player) });
        setRoom(joined);
      } catch (e) { setError(errorMessage(e, "No se pudo entrar en la revancha.")); }
      return;
    }
    setRematchLoading(true); setError(null);
    try {
      const created = await apiFetch<{ rematchCode: string }>(`/api/rooms/${encodeURIComponent(room.roomCode)}/rematch`, {
        method: "POST",
        body: JSON.stringify({ playerId: player.playerId, playerName: player.playerName, avatarColor: player.avatarColor }),
      });
      if (!created.rematchCode) throw new Error("El servidor no devolvió una sala de revancha.");
      setRematchCode(created.rematchCode);
      const joined = await apiFetch<Room>(`/api/rooms/${encodeURIComponent(created.rematchCode)}/join`, { method: "POST", body: JSON.stringify(player) });
      setRoom(joined); setCode(created.rematchCode); setAnswers({}); setSubmitted(false); setRoundKey("");
    } catch (e) { setError(errorMessage(e, "No se pudo crear la revancha.")); }
  ...
  if (room?.status === "finished") {
    ...
    return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.emoji}>🏆</Text><Text style={styles.title}>¡Partida terminada!</Text><Text style={styles.subtitle}>Resultados finales</Text>
      {ranked.map((p, i) => <View key={p.playerId} style={[styles.player, i === 0 && styles.winner]}><Text style={styles.playerName}>{i + 1}. {p.playerName}</Text><Text style={styles.playerScore}>{p.score ?? 0} pts</Text></View>)}
      {error && <Text style={styles.error}>{error}</Text>}
      <TouchableOpacity style={styles.rematch} disabled={rematchLoading} onPress={() => void requestRematch}>{rematchLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryText}>🔄 REVANCHA</Text>}</TouchableOpacity>
      <Text style={styles.rematchHint}>{rematchCode ? `Nueva sala: ${rematchCode}` : "El primer jugador que pulse crea la nueva sala para todos."}</Text>
      <TouchableOpacity style={styles.primary} onPress={onExit}><Text style={styles.primaryText}>Volver al inicio</Text></TouchableOpacity>
    </ScrollView></SafeAreaView>;
  }

  if (room) return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    <Text style={styles.title}>Sala {room.roomCode}</Text><Text style={styles.subtitle}>Comparte el código con tus amigos.</Text>
    <View style={styles.code}><Text style={styles.codeLabel}>CÓDIGO</Text><Text style={styles.codeValue}>{room.roomCode}</Text></View>
    <TouchableOpacity style={styles.shareButton} onPress={() => void shareRoom}><Text style={styles.shareText}>📤 Compartir invitación</Text></TouchableOpacity>
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.container}>
    ...
    <TouchableOpacity style={styles.primary} disabled={busy} onPress={() => void createRoom}>{busy ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryText}>＋ Crear sala</Text>}</TouchableOpacity>
    ...
    <TouchableOpacity style={styles.secondary} disabled={busy || code.trim().length < 4} onPress={() => void joinRoom}><Text style={styles.secondaryText}>Entrar en sala</Text></TouchableOpacity>
    {error && <Text style={styles.error}>{error}</Text>}<TouchableOpacity style={styles.back} onPress={onExit}><Text style={styles.backText}>Volver</Text></TouchableOpacity>
  </View></SafeAreaView>;
}
...
const styles = StyleSheet.create({
  ...
  primary: { minHeight: 52, borderRadius: 14, backgroundColor: "#151f63", alignItems: "center", justifyContent: "center", marginTop: 16 }, rematch: { minHeight: 56, borderRadius: 16, backgroundColor: "#e85d04", alignItems: "center", justifyContent: "center", marginTop: 18 }, primaryText: { color: "#fff", fontSize: 17, fontWeight: "800" },
  ...
  stopped: { backgroundColor: "#fff3cd", padding: 12, borderRadius: 12, marginBottom: 14, fontWeight: "700", color: "#7a5b00" }, waitCard: { marginTop: 16, padding: 18, borderRadius: 14, backgroundColor: "#e9ebf2" }, waitTitle: { fontSize: 17, fontWeight: "900", color: "#151f63" }, muted: { marginTop: 4, color: "#5d6170" }, waitText: { textAlign: "center", color: "#5d6170", marginTop: 22, fontWeight: "700" }, error: { color: "#b42318", marginTop: 14, fontWeight: "700", textAlign: "center" }, back: { marginTop: 20, alignItems: "center" }, emoji: { fontSize: 56, textAlign: "center", marginBottom: 10 }, winner: { borderWidth: 2, borderColor: "#151f63" }, rematchHint: { textAlign: "center", color: "#5d6170", marginTop: 10, fontSize: 13 },
});
