...
export function GameScreen({ onExit }: { onExit: () => void }) {
  ...
  const [categories, setCategories] = useState(STOP_CATEGORIES);
  ...
  const roundTime = useMemo(() => getModeDuration(mode), [mode]);
  const progress = secondsLeft / Math.max(1, roundTime);

  const loadDaily = useCallback(async () => {
    setLoadingDaily(true); setError(null);
    try { const challenge = await getDailyChallenge(getApiUrl(), "es") as DailyChallenge; setDaily(challenge); return challenge; }
  ...
  const finishRound = useCallback(async () => {
    ...
    try {
      const validation = await validateRound(getApiUrl(), buildValidationRequest(letter, "es", answers));
  ...
  const startGame = async (nextMode: StopMode = mode) => {
    ...
    if (nextMode === "diario") {
      const challenge = daily ?? await loadDaily();
      if (!challenge) return;
      if (challenge.completed) { setError("Ya has completado el Reto Diario de hoy."); return; }
      setLetter(challenge.letter); setCategories(challenge.categories); setAnswers(emptyAnswers(challenge.categories));
      setSecondsLeft(getModeDuration("normal"));
    } else {
  ...
  if (!running && !result) return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.container}>
    ...
    <Text style={styles.pageTitle}>Elige cómo jugar</Text><Text style={styles.pageSub}>Juego nativo iOS con las reglas vigentes de STOP.</Text>
    {MODES.map(item => <TouchableOpacity key={item} style={styles.modeCard} onPress={() => void startGame(item)} disabled={item === "diario" && loadingDaily} activeOpacity={0.85}>
      <View style={styles.modeIcon}><Text style={styles.modeEmoji}>{item === "normal" ? "🎯" : item === "rapido" ? "⚡" : item === "caos" ? "🌪️" : item === "random" ? "🎲" : "📅"}</Text></View>
      <View style={styles.modeContent}><Text style={styles.modeTitle}>{modeLabel(item)}</Text><Text style={styles.modeText}>{item === "normal" ? "La partida clásica de STOP." : item === "rapido" ? "Una ronda rápida." : item === "caos" ? "Ronda especial de Caos." : item === "random" ? "Modo aleatorio." : "El reto diario oficial."}</Text></View><Text style={styles.arrow}>›</Text>
    </TouchableOpacity>)}
  ...
  return <SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    <View style={styles.header}><TouchableOpacity onPress={onExit}><Text style={styles.back}>‹ Salir</Text></TouchableOpacity><Text style={styles.headerTitle}>STOP!</Text><Text style={styles.mode}>{modeLabel(mode)}</Text></View>
    <View style={styles.letterCard}><Text style={styles.letterLabel}>LETRA</Text><Text style={styles.letter}>{letter}</Text><View style={styles.timerTrack}><View style={[styles.timerFill,{width:`${Math.max(0,Math.min(1,progress))*100}%`}]} /></View><Text style={styles.timer}>{secondsLeft}s</Text></View>
    {mode === "diario" && daily && <View style={styles.infoBox}><Text style={styles.infoTitle}>Reto Diario · {daily.date}</Text><Text style={styles.infoText}>Este reto utiliza la letra y categorías oficiales asignadas para hoy.</Text></View>}
    <View style={styles.infoBox}><Text style={styles.infoTitle}>Escribe una palabra por categoría</Text><Text style={styles.infoText}>Todas deben empezar por {letter}. STOP termina la ronda y el servidor valida las respuestas.</Text></View>
