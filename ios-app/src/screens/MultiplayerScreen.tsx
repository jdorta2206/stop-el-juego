...
import { createRoom, getPublicRooms, getRoom, joinRoom, type Room } from '../multiplayer/roomApi';
...
export default function MultiplayerScreen() {
  ...
  const refreshRooms = async () => {
    try { setRooms((await getPublicRooms()).rooms ?? []); } catch { /* keep UI usable offline */ }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content}>
      ...
      <Text style={styles.subtitle}>Crea una sala o entra con un código.</Text>

      {!room && <>
        <Text style={styles.section}>Crear sala</Text>
        <View style={styles.row}>{[2,4,6,8].map((n) => <Pressable key={n} onPress={() => setMaxPlayers(n)} style={[styles.option, maxPlayers === n && styles.selected]}><Text style={maxPlayers === n ? styles.selectedText : styles.optionText}>{n} jugadores</Text></Pressable>)}</View>
        ...
        <Text style={styles.section}>Unirse</Text>
        <TextInput value={roomCode} onChangeText={setRoomCode} autoCapitalize="characters" autoCorrect={false} maxLength={8} placeholder="CÓDIGO DE SALA" style={styles.input} />
