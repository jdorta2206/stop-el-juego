    ...
    public static synchronized void preloadRewardedAd() {
        ...
        RewardedAd.load(instance, REAL_REWARDED_ID, new AdRequest.Builder().build(), new RewardedAdLoadCallback() {
            ...
            public void onAdFailedToLoad(@NonNull LoadAdError error) {
                loadingRewardedAd = false;
                Log.e(TAG, "Rewarded preload failed: code=" + error.getCode() + " domain=" + error.getDomain() + " message=" + error.getMessage());
                if (instance != null) instance.handler.postDelayed(Application::preloadRewardedAd, RETRY_DELAY_MS);
            }
        });
    }
    @Nullable
